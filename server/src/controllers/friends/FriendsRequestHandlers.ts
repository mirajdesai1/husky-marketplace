...
import { IUserProfile, IUserPublicProfile } from '../profile/ProfileRequestHandlers';
...
export async function friendsGetHandler(
    ...
    if (userProf) {
        friendProfs = await collection.find<IUserPublicProfile>({userId: {$in: userProf.friends}}).toArray();
    }
...
export async function friendsAcceptHandler(
    ...
    const collection = await ProfileCollection();
    const friend2Prof = await collection.findOne<IUserProfile>({ username: friend2Username });
    if (friend2Prof) {
        await collection.updateOne(
            { userId: friend1UserId },
            { $addToSet: { friends: friend2Username }, $pull: { pendingFriendRequests: friend2Prof.userId }
        });
    }

    const friend1ProfUpdated = await collection.findOne<IUserProfile>({ userId: friend1UserId });
    ...
    const friend2ProfUpdated = await collection.findOne<IUserProfile>({ username: friend2Username });
...
export async function friendsSendInviteHandler(
    ...
    await collection.updateOne({ username: friend2Username }, { $addToSet: { pendingFriendRequests: friend1UserId } });
